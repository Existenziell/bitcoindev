...
The block header is hashed twice with SHA-256 (try the [Hash tool](/interactive-tools/hash)) to produce the block hash, which must meet the difficulty target for the block to be valid. See the [Block Visualizer](/interactive-tools/block-visualizer) to explore the latest blocks and transactions.
